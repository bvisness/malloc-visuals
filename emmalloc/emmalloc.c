 ...
 *  - If EMMALLOC_MEMVALIDATE is defined, a large amount of extra checks are
 * done.
...
// Behavior of right shifting a signed integer is compiler implementation
// defined.
static_assert((((int32_t)0x80000000U) >> 31) == -1,
              "This malloc implementation requires that right-shifting a "
              "signed integer produces a sign-extending (arithmetic) shift!");

// Configuration: specifies the minimum alignment that malloc()ed memory
...
// <size:uint32_t> ..... <size:uint32_t> | <size:uint32_t> ..... <size:uint32_t>
// | <size:uint32_t> ..... <size:uint32_t> | .....

// That is, at the bottom and top end of each memory region, the size of that
// region is stored. That allows traversing the memory regions backwards and
// forwards. Because each allocation must be at least a multiple of 4 bytes, the
// lowest two bits of each size field is unused. Free regions are distinguished
// by used regions by having the FREE_REGION_FLAG bit present in the size field.
// I.e. for free regions, the size field is odd, and for used regions, the size
// field reads even.
#define FREE_REGION_FLAG 0x1u

// Attempts to malloc() more than this many bytes would cause an overflow when
// calculating the size of a region, therefore allocations larger than this are
// short-circuited immediately on entry.
...
typedef struct Region {
  ...
  size_t _at_the_end_of_this_struct_size; // do not dereference, this is present
                                          // for convenient struct sizeof()
                                          // computation only
} Region;
...
// In multithreaded builds, use a simple global spinlock strategy to
// acquire/release access to the memory allocator.
...
#define ALIGN_UP(ptr, alignment)                                               \
  ((uint8_t *)((((uintptr_t)(ptr)) + ((alignment)-1)) & ~((alignment)-1)))
#define HAS_ALIGNMENT(ptr, alignment)                                          \
  ((((uintptr_t)(ptr)) & ((alignment)-1)) == 0)

static_assert(IS_POWER_OF_2(MALLOC_ALIGNMENT),
              "MALLOC_ALIGNMENT must be a power of two value!");
static_assert(MALLOC_ALIGNMENT >= 4,
              "Smallest possible MALLOC_ALIGNMENT if 4!");
...
// For each of the buckets, maintain a linked list head node. The head node for
// each free region is a sentinel node that does not actually represent any free
// space, but the sentinel is used to avoid awkward testing against (if node ==
// freeRegionHeadNode) when adding and removing elements from the linked list,
// i.e. we are guaranteed that the sentinel node is always fixed and there, and
// the actual free region list elements start at freeRegionBuckets[i].next each.
static Region freeRegionBuckets[NUM_FREE_BUCKETS];

// A bitmask that tracks the population status for each of the 64 distinct
// memory regions: a zero at bit position i means that the free list bucket i is
// empty. This bitmask is used to avoid redundant scanning of the 64 different
// free region buckets: instead by looking at the bitmask we can find in
// constant time an index to a free region bucket that contains free memory of
// desired size.
...
// Smallest allocation size that is possible is 2*pointer size, since payload of
// each region must at least contain space to store the free region linked list
// prev and next pointers. An allocation size smaller than this will be rounded
// up to this size.
#define SMALLEST_ALLOCATION_SIZE (2 * sizeof(void *))

/* Subdivide regions of free space into distinct circular doubly linked lists,
where each linked list represents a range of free space blocks. The following
function compute_free_list_bucket() converts an allocation size to the bucket
index that should be looked at. The buckets are grouped as follows:
...
static_assert(NUM_FREE_BUCKETS == 64,
              "Following function is tailored specifically for "
              "NUM_FREE_BUCKETS == 64 case");
static int compute_free_list_bucket(size_t allocSize) {
  if (allocSize < 128)
    return (allocSize >> 3) - 1;
  int clz = __builtin_clz(allocSize);
  int bucketIndex = (clz > 19)
                        ? 110 - (clz << 2) + ((allocSize >> (29 - clz)) ^ 4)
                        : MIN(71 - (clz << 1) + ((allocSize >> (30 - clz)) ^ 2),
                              NUM_FREE_BUCKETS - 1);
...
static void link_to_free_list(Region *freeRegion) {
  ...
  int bucketIndex =
      compute_free_list_bucket(freeRegion->size - REGION_HEADER_SIZE);
...
static void dump_memory_regions() {
  ...
  while (root) {
    ...
    MAIN_THREAD_ASYNC_EM_ASM(out('Region block ' + ptrToString($0) + ' - ' +
                                 ptrToString($1) + ' (' + toString(Number($2)) +
                                 ' bytes):'),
                             r, lastRegionEnd, lastRegionEnd - (uint8_t *)r);
    while ((uint8_t *)r < lastRegionEnd) {
      MAIN_THREAD_ASYNC_EM_ASM(out('Region ' + ptrToString($0) + ', size: ' +
                                   toString(Number($1)) + ' (' +
                                   ($2 ? "used" : "--FREE--") + ')'),
      ...
      if (sizeFromCeiling != r->size) {
        MAIN_THREAD_ASYNC_EM_ASM(
            out('Corrupt region! Size marker at the end of the region does not match: ' +
                toString(Number($0))),
            sizeFromCeiling);
      }
      if (r->size == 0) {
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    while (fr != &freeRegionBuckets[i]) {
      MAIN_THREAD_ASYNC_EM_ASM(
          out('In bucket ' + $0 + ', free region ' + ptrToString($1) +
              ', size: ' + toString(Number($2)) + ' (size at ceiling: ' +
              toString(Number($3)) + '), prev: ' + ptrToString($4) +
              ', next: ' + ptrToString($5)),
  ...
  MAIN_THREAD_ASYNC_EM_ASM(
      out('Free bucket index map: ' + toString(Number($0)).toString(2) + ' ' +
          toString(Number($1)).toString(2)),
...
static int validate_memory_regions() {
  ...
  while (root) {
    ...
    if (!debug_region_is_consistent(r)) {
      MAIN_THREAD_ASYNC_EM_ASM(
          err('Used region ' + ptrToString($0) + ', size: ' +
              toString(Number($1)) + ' (' + ($2 ? "used" : "--FREE--") +
              ') is corrupt (size markers in the beginning and at the end of the region do not match!)'),
          r, r->size, region_ceiling_size(r) == r->size);
      return 1;
    }
    uint8_t *lastRegionEnd = root->endPtr;
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    Region *fr = freeRegionBuckets[i].next;
    while (fr != &freeRegionBuckets[i]) {
      if (!debug_region_is_consistent(fr) || !region_is_free(fr) ||
          fr->prev != prev || fr->next == fr || fr->prev == fr) {
        MAIN_THREAD_ASYNC_EM_ASM(
            out('In bucket ' + $0 + ', free region ' + ptrToString($1) +
                ', size: ' + toString(Number($2)) + ' (size at ceiling: ' +
                toString(Number($3)) + '), prev: ' + ptrToString($4) +
                ', next: 0x' + ptrToString($5) + ' is corrupt!'),
            i, fr, fr->size, size_of_region_from_ceiling(fr), fr->prev,
            fr->next);
        return 1;
      }
      prev = fr;
...
static bool claim_more_memory(size_t numBytes) {
  ...
  MAIN_THREAD_ASYNC_EM_ASM(
      out('claim_more_memory(numBytes=' + Number($0) + ')'), numBytes);
#endif
  ...
  // allocates memory at. Otherwise we will not properly interpret returned
  // memory to form a seamlessly contiguous region with earlier root regions,
  // which would lead to inefficiently treating the sbrk()ed region to be a new
  // disjoint root region.
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(out('claim_more_memory: claimed ' + ptrToString($0) +
                               ' - ' + ptrToString($1) + ' (' + Number($2) +
                               ' bytes) via sbrk()'),
  ...
  // If we are the sole user of sbrk(), it will feed us continuous/consecutive
  // memory addresses - take advantage of that if so: instead of creating two
  // disjoint memory regions blocks, expand the previous one to a larger size.
  uint8_t *previousSbrkEndAddress =
      listOfAllRegions ? listOfAllRegions->endPtr : 0;
  if (startPtr == previousSbrkEndAddress) {
#ifdef EMMALLOC_VERBOSE
    MAIN_THREAD_ASYNC_EM_ASM(err(
    ...
    // Two scenarios, either the last region of the previous block was in use,
    // in which case we need to create a new free region in the newly allocated
    // space; or it was free, in which case we can extend that region to cover a
    // larger size.
    if (region_is_free(prevRegion)) {
      size_t newFreeRegionSize =
          (uint8_t *)endSentinelRegion - (uint8_t *)prevRegion;
      unlink_from_free_list(prevRegion);
      create_free_region(prevRegion, newFreeRegionSize);
      link_to_free_list(prevRegion);
      return true;
    }
    // else: last region of the previous block was in use. Since we are joining
    // two consecutive sbrk() blocks, we can swallow the end sentinel of the
    // previous block away.
    startPtr -= sizeof(Region);
  } else {
    // Unfortunately some other user has sbrk()ed to acquire a slab of memory
    // for themselves, and now the sbrk()ed memory we got is not contiguous with
    // our previous managed root regions. So create a new root region at the
    // start of the sbrk()ed heap block.
#ifdef EMMALLOC_VERBOSE
    MAIN_THREAD_ASYNC_EM_ASM(err(
    ...
    newRegionBlock->next =
        listOfAllRegions; // Pointer to next region block head
    newRegionBlock->endPtr =
        endPtr; // Pointer to the end address of this region block
...
__attribute__((constructor(47))) static void initialize_emmalloc_heap() {
...
#pragma clang loop unroll(disable)
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    freeRegionBuckets[i].prev = freeRegionBuckets[i].next =
        &freeRegionBuckets[i];
  }
...
static void *attempt_allocate(Region *freeRegion, size_t alignment,
                              size_t size) {
  ASSERT_MALLOC_IS_ACQUIRED();

#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(out('attempt_allocate(freeRegion=' +
                               ptrToString($0) + ',alignment=' + Number($1) +
                               ',size=' + Number($2) + ')'),
  ...
  // First, we should check if the free region has enough of payload bytes
  // contained in it to accommodate the new allocation. This check needs to take
  // account the requested allocation alignment, so the payload memory area
  // needs to be rounded upwards to the desired alignment.
  ...
  // We have enough free space, so the memory allocation will be made into this
  // region. Remove this free region from the list of free regions: whatever
  // slop remains will be later added back to the free region pool.
  unlink_from_free_list(freeRegion);

  // Before we proceed further, fix up the boundary between this and the
  // preceding region, so that the boundary between the two regions happens at a
  // right spot for the payload to be aligned.
  if (payloadStartPtr != payloadStartPtrAligned) {
    Region *prevRegion = prev_region((Region *)freeRegion);
    // We never have two free regions adjacent to each other, so the region
    // before this free region should be in use.
  ...
  // Next, we need to decide whether this region is so large that it should be
  // split into two regions, one representing the newly used memory area, and at
  // the high end a remaining leftover free area. This splitting to two is done
  // always if there is enough space for the high end to fit a region. Carve
  // 'size' bytes of payload off this region. So, [sz prev next sz] becomes [sz
  // payload sz] [sz prev next sz]
  if (sizeof(Region) + REGION_HEADER_SIZE + size <= freeRegion->size) {
    // There is enough space to keep a free region at the end of the carved out
    // block
    // -> construct the new block
    Region *newFreeRegion =
        (Region *)((uint8_t *)freeRegion + REGION_HEADER_SIZE + size);
    create_free_region(newFreeRegion,
                       freeRegion->size - size - REGION_HEADER_SIZE);
    ...
    // There is not enough space to split the free memory region into used+free
    // parts, so consume the whole region as used memory, not leaving a free
    // memory region behind. Initialize the free region as used by resetting the
    // ceiling size to the same value as the size at bottom.
    ((size_t *)((uint8_t *)freeRegion + freeRegion->size))[-1] =
        freeRegion->size;
  }
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(
      out('attempt_allocate - succeeded allocating memory, region ptr=' +
          ptrToString($0) + ', align=' + $1 + ', payload size=' +
          toString(Number($2)) + ' bytes)'),
      freeRegion, alignment, size);
#endif
...
static size_t validate_alloc_size(size_t size) {
  ...
  // Allocation sizes must be a multiple of pointer sizes, and at least
  // 2*sizeof(pointer).
  size_t validatedSize = size > SMALLEST_ALLOCATION_SIZE
                             ? (size_t)ALIGN_UP(size, sizeof(Region *))
                             : SMALLEST_ALLOCATION_SIZE;
  assert(validatedSize >= size); // 32-bit wraparound should not occur, too
                                 // large sizes should be stopped before
...
static void *allocate_memory(size_t alignment, size_t size) {
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(out('allocate_memory(align=' + $0 + ', size=' +
                               toString(Number($1)) + ' bytes)'),
                           alignment, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
    MAIN_THREAD_ASYNC_EM_ASM(
        out('Allocation failed: alignment not power of 2!'));
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
    MAIN_THREAD_ASYNC_EM_ASM(
        out('Allocation failed: attempted allocation size is too large: ' +
            toString(Number($0)) + 'bytes! (negative integer wraparound?)'),
        size);
#endif
    return 0;
  }
  ...
  // Attempt to allocate memory starting from smallest bucket that can contain
  // the required amount of memory. Under normal alignment conditions this
  // should always be the first or second bucket we look at, but if performing
  // an allocation with complex alignment, we may need to look at multiple
  // buckets.
  int bucketIndex = compute_free_list_bucket(size);
  BUCKET_BITMASK_T bucketMask = freeRegionBucketsUsed >> bucketIndex;

  // Loop through each bucket that has free regions in it, based on bits set in
  // freeRegionBucketsUsed bitmap.
  while (bucketMask) {
    ...
    if (freeRegion != &freeRegionBuckets[bucketIndex]) {
      ...
      // We were not able to allocate from the first region found in this
      // bucket, so penalize the region by cycling it to the end of the doubly
      // circular linked list. (constant time) This provides a randomized
      // guarantee that when performing allocations of size k to a bucket of
      // [k-something, k+something] range, we will not always attempt to satisfy
      // the allocation from the same available region at the front of the list,
      // but we try each region in turn.
      unlink_from_free_list(freeRegion);
      prepend_to_free_list(freeRegion, &freeRegionBuckets[bucketIndex]);
      // But do not stick around to attempt to look at other regions in this
      // bucket - move to search the next populated bucket index if this did not
      // fit. This gives a practical "allocation in constant time" guarantee,
      // since the next higher bucket will only have regions that are all of
      // strictly larger size than the requested allocation. Only if there is a
      // difficult alignment requirement we may fail to perform the allocation
      // from a region in the next bucket, and if so, we keep trying higher
      // buckets until one of them works.
      ++bucketIndex;
      bucketMask >>= 1;
    } else {
    ...
    // Instead of recomputing bucketMask from scratch at the end of each loop,
    // it is updated as we go, to avoid undefined behavior with (x >> 32)/(x >>
    // 64) when bucketIndex reaches 32/64, (the shift would come out as a no-op
    // instead of 0).
    assert((bucketIndex == NUM_FREE_BUCKETS && bucketMask == 0) ||
           (bucketMask == freeRegionBucketsUsed >> bucketIndex));
  }

  // None of the buckets were able to accommodate an allocation. If this happens
  // we are almost out of memory. The largest bucket might contain some suitable
  // regions, but we only looked at one region in that bucket, so as a last
  // resort, loop through more free regions in the bucket that represents the
  // largest allocations available. But only if the bucket representing largest
  // allocations available is not any of the first thirty buckets, these
  // represent allocatable areas less than <1024 bytes - which could be a lot of
  // scrap. In such case, prefer to sbrk() in more memory right away.
  int largestBucketIndex =
      NUM_FREE_BUCKETS - 1 - __builtin_clzll(freeRegionBucketsUsed);
  // freeRegion will be null if there is absolutely no memory left. (all buckets
  // are 100% used)
  Region *freeRegion =
      freeRegionBucketsUsed ? freeRegionBuckets[largestBucketIndex].next : 0;
  // The 30 first free region buckets cover memory blocks < 2048 bytes, so skip
  // looking at those here (too small)
  if (freeRegionBucketsUsed >> 30) {
    // Look only at a constant number of regions in this bucket max, to avoid
    // bad worst case behavior. If this many regions cannot find free space, we
    // give up and prefer to sbrk() more instead.
  ...
  assert(numBytesToClaim > size); // 32-bit wraparound should not happen here,
                                  // allocation size has been validated above!
  ...
  // also sbrk() failed, we are really really constrained :( As a last resort,
  // go back to looking at the bucket we already looked at above, continuing
  // where the above search left off - perhaps there are regions we overlooked
  // the first time that might be able to satisfy the allocation.
  if (freeRegion) {
    while (freeRegion != &freeRegionBuckets[largestBucketIndex]) {
      void *ptr = attempt_allocate(freeRegion, alignment, size);
      if (ptr) {
...
void emmalloc_free(void *ptr) {
  ...
  if (size < sizeof(Region) || !region_is_in_use(region)) {
    if (debug_region_is_consistent(region)) {
      // LLVM wasm backend bug: cannot use MAIN_THREAD_ASYNC_EM_ASM() here, that
      // generates internal compiler error Reproducible by running e.g.
      // other.test_alloc_3GB
      EM_ASM(err('Double free at region ptr ' + ptrToString($0) +
                 ', region->size: ' + ptrToString($1) +
                 ', region->sizeAtCeiling: ' + ptrToString($2) + ')'),
             region, size, region_ceiling_size(region));
    } else {
      MAIN_THREAD_ASYNC_EM_ASM(
          err('Corrupt region at region ptr ' + ptrToString($0) +
              ' region->size: ' + ptrToString($1) +
              ', region->sizeAtCeiling: ' + ptrToString($2) + ')'),
          region, size, region_ceiling_size(region));
    }
...
static int attempt_region_resize(Region *region, size_t size) {
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(out('attempt_region_resize(region=' +
                               ptrToString($0) + ', size=' +
                               toString(Number($1)) + ' bytes)'),
                           region, size);
#endif

  // First attempt to resize this region, if the next region that follows this
  // one is a free region.
  Region *nextRegion = next_region(region);
  ...
  if (nextRegion->size != sizeAtCeiling) { // Next region is free?
    ...
    if (newNextRegionStartPtr + sizeof(Region) <= nextRegionEndPtr) {
      unlink_from_free_list(nextRegion);
      create_free_region(newNextRegionStartPtr,
                         nextRegionEndPtr - newNextRegionStartPtr);
    ...
    // Next region is an used region - we cannot change its starting address.
    // However if we are shrinking the size of this region, we can create a new
    // free region between this and the next used region.
    if (size + sizeof(Region) <= region->size) {
      ...
      // Caller was asking to shrink the size, but due to not being able to fit
      // a full Region in the shrunk area, we cannot actually do anything. This
      // occurs if the shrink amount is really small. In such case, just call it
...
void *emmalloc_aligned_realloc(void *ptr, size_t alignment, size_t size) {
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(out('aligned_realloc(ptr=' + ptrToString($0) +
                               ', alignment=' + $1 + ', size=' +
                               toString(Number($2))),
                           ptr, alignment, size);
#endif
  ...
  // aligned_realloc() cannot be used to ask to change the alignment of a
  // pointer.
  ...
  // First attempt to resize the given region to avoid having to copy memory
  // around
  ...
  // If resize failed, we must allocate a new region, copy the data over, and
  // then free the old region.
  void *newptr = emmalloc_memalign(alignment, size);
  if (newptr) {
    memcpy(newptr, ptr, MIN(size, region->size - REGION_HEADER_SIZE));
    free(ptr);
  }
  // N.B. If there is not enough memory, the old memory block should not be
  // freed and null pointer is returned.
  return newptr;
}
EMMALLOC_ALIAS(aligned_realloc, emmalloc_aligned_realloc);

// realloc_try() is like realloc(), but only attempts to try to resize the
// existing memory area. If resizing the existing memory area fails, then
// realloc_try() will return 0 (the original memory block is not freed or
// modified). If resizing succeeds, previous memory contents will be valid up to
// min(old length, new length) bytes.
void *emmalloc_realloc_try(void *ptr, size_t size) {
  ...
  int success =
      acquire_and_attempt_region_resize(region, size + REGION_HEADER_SIZE);
#ifdef __EMSCRIPTEN_TRACING__
  if (success) {
    emscripten_trace_record_reallocation(ptr, ptr, size);
  }
...
// emmalloc_aligned_realloc_uninitialized() is like aligned_realloc(), but old
// memory contents will be undefined after reallocation. (old memory is not
// preserved in any case)
void *emmalloc_aligned_realloc_uninitialized(void *ptr, size_t alignment,
                                             size_t size) {
  if (!ptr) {
    return emmalloc_memalign(alignment, size);
  }
  ...
  // If resize failed, drop the old region and allocate a new region. Memory is
  // not copied over
  free(ptr);
  return emmalloc_memalign(alignment, size);
}
...
// will be undefined after reallocation. (old memory is not preserved in any
// case)
...
struct mallinfo emmalloc_mallinfo() {
  ...
  // size blocks. (subtract one from each, since there is a sentinel node in
  // each list)
  info.ordblks =
      count_linked_list_size(&freeRegionBuckets[NUM_FREE_BUCKETS - 1]) - 1;
  ...
  // This is the maximum number of bytes that could ideally be released by
  // malloc_trim(3).
  Region *lastActualRegion =
      prev_region((Region *)(listOfAllRegions->endPtr - sizeof(Region)));
  ...
  while (root) {
    ...
    while ((uint8_t *)r < lastRegionEnd) {
      ...
      if (region_is_free(r)) {
        ...
        info.fordblks +=
            region_payload_end_ptr(r) - region_payload_start_ptr(r);
...
// Note! This function is not fully multithreading safe: while this function is
// running, other threads should not be allowed to call sbrk()!
static int trim_dynamic_heap_reservation(size_t pad) {
  ASSERT_MALLOC_IS_ACQUIRED();

  if (!listOfAllRegions) {
    return 0; // emmalloc is not controlling any dynamic memory at all - cannot
              // release memory.
  }
  ...
  assert(lastMemoryRegionSize ==
         16); // // The last memory region should be a sentinel node of exactly
              // 16 bytes in size.
  Region *endSentinelRegion =
      (Region *)(previousSbrkEndAddress - sizeof(Region));
  Region *lastActualRegion = prev_region(endSentinelRegion);

  // Round padding up to multiple of 4 bytes to keep sbrk() and memory region
  // alignment intact. Also have at least 8 bytes of payload so that we can form
  // a full free region.
  ...
  if (!region_is_free(lastActualRegion) ||
      lastActualRegion->size <= newRegionSize) {
    return 0; // Last actual region is in use, or caller desired to leave more
              // free memory intact than there is.
  }
  ...
  assert(oldSbrk == previousSbrkEndAddress); // Another thread should not have
                                             // raced to increase sbrk() on us!
...
size_t emmalloc_compute_free_dynamic_memory_fragmentation_map(
...
void emmalloc_dump_free_dynamic_memory_fragmentation_map() {
  size_t freeMemorySizeMap[32];
  size_t numFreeMemoryRegions =
      emmalloc_compute_free_dynamic_memory_fragmentation_map(freeMemorySizeMap);
